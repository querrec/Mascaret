...
public class InstanceLoader 
{
	...
	public InstanceLoader(AgentPlateform agentPlateform,Environment environment, string filename, bool toRun)
	{
		...
		System.Console.WriteLine( " ##################FIN INSTANCES " );
	}
	...
	{
		System.Console.WriteLine(" ########### ------ Parsing Instances ------ ########");
		...
		else
		{
			...
			{
				System.Console.WriteLine(" --------------------------------------------------  Node name : " + child.Name.LocalName);
	...
	{
		System.Console.WriteLine(" ####### Create entity ######## ");
		...
		//Debug.Log(" -------> " + nodeName + " : " + className);
		
		classe = _findClass(className);
		if (classe==null)
		{
			System.Console.WriteLine( "La classe " + className + " n'a pas été trouvée : L'entité "+ nodeName + " ne sera pas créée" );
			return e;
		}
		EntityClass entityClasse=(EntityClass)classe;
		if(entityClasse==null)
		{
			System.Console.WriteLine( "La classe " + className + " n'est pas une classe d'entité : L'entité "+ nodeName + " ne sera pas créée" );
			return e;
		}
		...
		foreach (XElement child in node.Elements() )
		{
			...
			else if (child.Name.LocalName == "Shape")
			{
				...
				*/
				...
				//UnityShapeSpecification specif = (UnityShapeSpecification)ScriptableObject.CreateInstance("UnityShapeSpecification");
				//specif.instantiate(entity.name+"_"+shapeName,url,movable,recursive,shader);
				//entity.getProperty(shapeName).addValue(specif);
				entity.setActiveShape(shapeName);
				//entity.ActiveShape = specif;
				//if (specif.EntityGO != null)
				{
					//EntityGo ego = (EntityGo)(specif.EntityGO.GetComponent("EntityGo"));
			...
			else if (child.Name.LocalName == "Shapes")
			{
				...
				foreach(XElement shapeNode in child.Elements())
				{
					if(shapeNode.Name.LocalName=="Shape")
					{
						...
						if(url.Length==0)
						{
							System.Console.WriteLine( "Fichier " + attr.Value + " non trouvé pour l'entity "+ entity.name );
						}
	...
	protected void _readGeometryEntity(XElement node, Entity entity)
	{
		...
		foreach(XElement child in node.Elements())
		{
			if (child.Name.LocalName == "Position" || child.Name.LocalName == "Location")
			{
				...
				if(attr!=null) 
                {
                   ...
                   if (instance!=null) 
                   {
						...
						if(attr!=null) 
                         {
                            ...
                            else System.Console.WriteLine( "Erreur : " + entPos.name + " has no point : " + pointName + " ! " );
                         }
                         else
                         {
                      ...
                      else System.Console.WriteLine( " Error : " + entityName + " is not an entity ! " );
                   }
                   else System.Console.WriteLine( " Error : " + entityName + " not found ! " );
			...
			else if (child.Name.LocalName == "Shape")
			{
				...
				//UnityShapeSpecification specif= (UnityShapeSpecification)ScriptableObject.CreateInstance("UnityShapeSpecification");
				//specif.instantiate(name,url,movable,recursive,shader);
	...
	protected OrganisationalEntity  _createOrganisation(XElement node)
	{
		...
		System.Console.WriteLine("CREATE ORGANISATION : " + name);
		...
		foreach(XElement struc in node.Elements())
		{
			if (struc.Name.LocalName == "Structure")
			{
				...
				if (structOrg==null)
					System.Console.WriteLine( "Erreur : Pas de structure pour l'organisation : " + name );
				else
				{
					...
					foreach(XElement affects in node.Elements())
					{
						if (affects.Name.LocalName == "Affect")
						{
							...
							if (attr!=null && attr.Value == "Role")
							{
								...
								if (role == null)
									System.Console.WriteLine( "Erreur, Role : " + roleName + " n'appartient pas a l'organisation : " + structName );
								else
								{
							...
							else if (attr!=null && attr.Value == "Ressource")
							{
								...
								if (entite==null) System.Console.WriteLine( "  #### Entity " + entityName + " non trouve ..." );
								...
								if (res!=null) System.Console.WriteLine( "  Ressource trouve ..." );
								else System.Console.WriteLine( " Ressource non trouve ..." );
	...
	protected VirtualHuman _createHuman(XElement node)
	{
		...
		System.Console.WriteLine(" ###################################################### CREATE VIRTUAL HUMAN");
		...
		if (classe==null)
		{
			System.Console.WriteLine( "La classe " + className + " n'a pas été trouvée : L'agent "+ nodeName+ " ne sera pas créé" );
			return h;
		}
		VirtualHumanClass vhClasse=(VirtualHumanClass)classe;
		if(vhClasse==null)
		{
			System.Console.WriteLine( "La classe " + className + " n'est pas une classe d'humain virtuel : L'agent "+ nodeName + " ne sera pas créé" );
			return h;
		}
		...
		foreach(XElement child in node.Elements())
		{
			if (child.Name.LocalName == "Face")
			{
				...
				//System.Console.WriteLine(h.Body.Face.FdpDataFile + " / " + h.Body.Face.IndiceFile);
			...
			else if(child.Name.LocalName == "Attribute")
			{
				...
				{
					System.Console.WriteLine( "The attribute " + attr.Value + " specified on the entity " + h.name + " doesn't exist");
				}
			}
			else if(child.Name.LocalName == "ExecuteOperation")
			{
				...
				foreach(Parameter currentParam in operation.Parameters)
				{
					...
					if(!found)
					{
						System.Console.WriteLine( "Can't find the value of the parameter " + currentParam.name + " when launching operation "
								+ operation.name + " on entity "+ h.name );
						continue;
					}
					strVal.Replace("+"," ");
					try
					...
					{
						System.Console.WriteLine( "Can't find the entity " + strVal + " when launching operation "
								+ operation.name + " on entity "+ h.name );
					}
				}
				attr = (XAttribute) child.Attribute("name");
	...
	protected void _addRelations()
	{
		...
		foreach (Relation currentRelation in relations)
		{
			...
			{
				//Debug.Log ("Relation : " + currentRelation.e.getFullName() + " . " + currentRelation.property + " = " + currentRelation.rValue);
			  	Property p = currentRelation.e.getProperty(currentRelation.property).DefiningProperty;
				//Debug.Log(p.Type.getFullName());
